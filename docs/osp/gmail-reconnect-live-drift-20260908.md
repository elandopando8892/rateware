...
## Reconnect completed and Crane source routing result

- The bounded OAuth state was consumed at `2026-09-09 02:21:13 UTC` for the existing XBFMX mailbox row. Database readback confirmed the exact account `carriers@xbfreight.com`, scopes `gmail.readonly`, `gmail.send`, `userinfo.email` and `openid`, a refresh token, `last_error = null`, and no second mailbox connection.
- The temporary bridge was removed immediately after callback. Restored shared runtime hashes are `provider-gmail-intake-api` v187 `9867c4e70377cdc5eef1cf9b593fd96a198a6630161ebe21fa44251fa5a03e75` and `provider-gmail-oauth-callback` v177 `938f558beb64f6e93a0059c337aeca52ed9aa5adc05ac58361b6c691206a8005`. Cron job 3 remained disabled.
- Authenticated production OSP preflight queried `in:anywhere subject:"Documentacion de alta para proveedores" after:2026/03/12 before:2026/03/13`. Instrumented HTTP evidence returned 200 with zero candidates, `checkpoint_unchanged = true`, `persisted = false` and `outbound_enabled = false`.
- The preflight refreshed the connection successfully; subsequent readback advanced `token_expires_at` to `2026-09-09 06:24:25 UTC` and `updated_at` to `2026-09-09 05:24:26 UTC`. Gmail authentication is therefore operational.
- The preserved Crane discovery record states that the original and amendment were read from the Sales Gmail connector, not from the carriers mailbox. Zero candidates is a source-routing result, not an OAuth failure. Do not import, fabricate or claim an inbox match; route the original thread into the governed carriers intake or use an explicitly approved preserved-source ingestion path.

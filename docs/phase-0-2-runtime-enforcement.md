...
- `shadow`: evaluate the canonical resolver, emit `TENANT_ENFORCEMENT_SHADOW_REJECT` on rejection, and preserve current access.
...
Unknown values resolve to `disabled`; there is no accidental fail-open transition into `shadow` or `required`.

...
- 67 active PostgreSQL/RPC signatures;
- 351 active discovered surfaces;
- 244 `rateware-api` actions.

The contract retains 353 rows because two historically counted RPCs are now proven absent after committed `DROP FUNCTION` statements:
...
H07 records both signatures as intentionally `removed`, with `public.rfx_benchmark_candidate_rate_ids(text,uuid,integer)` as their functional replacement. The repository history, current API call, stability guard, and a read-only production catalog probe confirm that the old signatures are absent while the service-role-only replacement remains active. Historical rows remain in the 353-record contract; active discovery remains 351.

## Decision and lifecycle boundary

The contract preserves 256 `pending_human_approval`, 27 `explicitly_allowed`, and 70 `internal_only` rows. No pending surface is converted to allowed.

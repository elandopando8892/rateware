...
Lineage labels and filenames are context, not identifiers: a structured reference must include a textual `id`, `raw_upload_id`, or `rate_staging_id`. Empty objects, currency-only metadata, and boolean-only placeholders do not count as usable observations. Monetary fields use the explicit BI metric contract rather than name fragments. Every monetary observation must carry currency in its own evidence tree unless the result declares one source-wide currency; a currency on one sibling row never covers another. Nested arrays are inspected without silent item truncation, and invalid amounts or evidence exceeding the bounded inspection budget keep the brief blocked.

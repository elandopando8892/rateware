...
describe('CaseFormWorkspace', () => {
  it('shows saved candidate counts without calling them approved memory', async () => {
    renderRoute({ getCaseFormWorkspace: vi.fn().mockResolvedValue({ ...workspace, answerMemory: { pendingCount: 4, unboundCount: 1, staleCount: 2, approvedForReuse: false } }), saveCaseFormDraft: vi.fn(), acceptCaseFormMapping: vi.fn(), correctCaseFormMapping: vi.fn(), submitCaseFormForReview: vi.fn() });
    expect(await screen.findByText('Saved answers awaiting memory review')).toBeInTheDocument();
    expect(screen.getByText(/4 captured candidates/)).toBeInTheDocument();
    expect(screen.getByText(/not approved reusable facts/)).toBeInTheDocument();
  });

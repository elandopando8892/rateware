        ...
        <section className="profile-review-queue panel" aria-labelledby="profile-review-title">
          ...
          {selectedCandidate && selectedField ? <section className="profile-decision-panel" aria-labelledby="profile-decision-title">
            ...
            {selectedCandidate.ownership === 'owned' ? <>
              ...
              {['pending', 'corrected'].includes(selectedCandidate.field_status) ? <div className="decision-actions">
                {selectedCandidate.field_status === 'corrected' ? <p role="status">Amend this open correction. The previous value, reviewer and note remain in the audit history. Approval is a separate action.</p> : null}
                ...
                {selectedCandidate.field_status === 'pending' ? <button type="button" className="danger-action" disabled={!noteReady || mutation.isPending} onClick={() => mutation.mutate(() => client.decideProfileReviewField({ reviewId: selectedCandidate.review_id, fieldId: selectedCandidate.review_field_id, expectedRevision: selectedCandidate.review_revision, decision: 'rejected', decisionNote, reviewerValue: null }))}>Reject evidence</button> : null}
              </div> : null}

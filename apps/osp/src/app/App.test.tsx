  ...
  return {
    ...
    getCorporateProfile: vi.fn(async () => ({
      ...
      entities: [{
        ...
        fields: [{ code: 'tax_identifier', label: 'Tax identifier', display_value: 'On file', verification_status: 'verified' as const, sensitivity: 'restricted' as const, support_status: 'verified_match' as const, evidence_candidate_count: '1', reviewed_candidate_count: '1', review_candidates: [] }], promotion_candidates: [], evidence: [],
      }],
    })),
    claimProfileReview: vi.fn(async () => { throw new Error('not used'); }),
    decideProfileReviewField: vi.fn(async () => { throw new Error('not used'); }),
    finalizeProfileReview: vi.fn(async () => { throw new Error('not used'); }),
    ...
    saveRequestManifestReview: vi.fn(async () => { throw new Error('not used'); }),
    getRequestKnowledgeWorkspace: vi.fn(async () => { throw new Error('not used'); }),
    promoteRequestKnowledge: vi.fn(async () => { throw new Error('not used'); }),
    ...
    getManualConversionCandidate: vi.fn(async () => { throw new Error('not used'); }),
...
describe('App authentication and routing', () => {
  ...
  it('routes Sales to the exact server-authorized payload without exposing private material', async () => {
    ...
    vi.mocked(api.getApprovalCommunicationsWorkspace).mockResolvedValue({
      ...
      outbound: {
        ...
        bodyText: 'Ready for exact Sales review.',
  ...
  it('never renders or queries stale router context when session identity and client switch', async () => {
    ...
    const clientB: OspClient = {
      ...
      saveRequestManifestReview: vi.fn(() => pending),
      ...
      getManualConversionSource: vi.fn(() => pending), getManualConversionCandidate: vi.fn(() => pending), uploadCaseConversion: vi.fn(() => pending), recordManualConversionReview: vi.fn(() => pending), listManualConversionCandidates: vi.fn(() => pending),

  ...
  return {
    ...
    getCorporateProfile: vi.fn(async () => ({
      ...
      entities: [{
        ...
        fields: [{ code: 'tax_identifier', label: 'Tax identifier', display_value: 'On file', verification_status: 'verified' as const, sensitivity: 'restricted' as const, support_status: 'verified_match' as const, evidence_candidate_count: '1', reviewed_candidate_count: '1', review_candidates: [] }], evidence: [],
      }],
    })),
...
describe('App authentication and routing', () => {
  ...
  it('never renders or queries stale router context when session identity and client switch', async () => {
    ...
    const clientB: OspClient = {
      ...
      claimProfileReview: vi.fn(() => pending), decideProfileReviewField: vi.fn(() => pending), finalizeProfileReview: vi.fn(() => pending),

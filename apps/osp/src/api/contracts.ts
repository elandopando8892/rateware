...
const FormDefinitionSchema = z.discriminatedUnion('kind', [
  ...
  z.strictObject({ kind: z.literal('repeating_table'), columns: z.array(z.strictObject({ id: z.string().min(1).max(64), label: z.string().min(1).max(128), valueType: z.enum(['text', 'number', 'date', 'email', 'phone']), required: z.boolean().optional() })).min(1).max(20), maxRows: z.number().int().min(1).max(100), minRows: z.number().int().min(0).max(100).optional(), uniqueBy: z.string().min(1).max(64).optional() }).refine((table) => (table.minRows ?? 0) <= table.maxRows && (table.uniqueBy === undefined || table.columns.some((column) => column.id === table.uniqueBy))),

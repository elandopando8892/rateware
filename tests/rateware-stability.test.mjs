...
assert.match(carrierListTemplatesSource, /const request = Object\.freeze\(\{[\s\S]+lightweight: true,[\s\S]+limit: CRM_PAGE_SIZE,[\s\S]+offset: requestedOffset[\s\S]+fetchVendors\(request\)/, "Template membership search should use immutable bounded paginated Carrier CRM reads");

...
assert.match(rfxBidSource, /validatePercentIssue\(draft\.marksman_margin_pct, "bid-marksman-margin", "MARKSMAN margin %", \{ required: true, procurementRange: true \}\)/, "Carrier portal should enforce MARKSMAN margin range for cost-plus");
assert.match(rfxBidSource, /validatePercentIssue\(draft\.carrier_share_pct, "bid-carrier-share", "Carrier share %", \{ required: true, procurementRange: true \}\)/, "Carrier portal should enforce carrier share range for billing share");

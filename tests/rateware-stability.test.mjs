...
assert.match(rfxEventsSource, /alreadyInRfx: existingParticipantIds\.has\(String\(vendor\.id \|\| ""\)\)/, "Carrier fit should retain existing RFx participants as visible status rows");
assert.match(rfxEventsSource, /alreadyInRfx \|\| selected \|\| !contactReady \? "disabled"/, "Carrier fit should never offer an existing RFx participant for selection again");
...
assert.doesNotMatch(generateOutreachDraftsSource, /publishOutreachTemplateToWhatsapp\(/, "Generating a WhatsApp queue must not publish or refresh a Meta notifier");
assert.match(generateOutreachDraftsSource, /if \(wantsDirectWhatsapp\) \{[\s\S]+whatsappTemplateMapping\(/, "Generating a WhatsApp queue should read the existing Meta notifier mapping");
...
assert.match(apiSource, /whatsapp_notifier: whatsappNotifier/, "Draft generation should return the read-only Meta notifier state");
...
assert.match(vendorsSource, /function updateVendorMetricsForRows\(rows = \[\], total = rows\.length\)[\s\S]+function updateVendorMetrics\(\)[\s\S]+updateVendorMetricsForRows\(rows, vendorTotalCount \|\| rows\.length\)/, "Carrier CRM metrics should support both directory and procurement-funnel scopes");
assert.match(vendorsSource, /renderVendorFunnelMetrics\(result\.summary \|\| \{\}\);[\s\S]+updateVendorMetricsForRows\(vendorFunnelRows, result\.summary\?\.total \?\? vendorFunnelRows\.length\)/, "Carrier CRM funnel should sync the visible header metrics to the loaded procurement scope");
assert.match(vendorsSource, /status: "Loading procurement funnel",[\s\S]+busy: true,[\s\S]+status: warning \|\| `\$\{result\.summary\?\.total \|\| 0\} procurement vendor\(s\) loaded`[\s\S]+busy: false/, "Carrier CRM funnel should expose loading and loaded state through the Platform 55 shell");
...
assert.match(rfxEventsHtml, /<span>RFx status<\/span>/, "Carrier Fit should integrate current RFx membership directly in the results table");
assert.match(rfxEventsHtml, /id="rfx-outreach-audience-builder"[\s\S]*?hidden/, "Invitation-status data may remain available to Message and Delivery without rendering a competing Carrier Fit surface");
assert.doesNotMatch(rfxEventsHtml, /rfx-open-this-rfx-drawer|rfx-this-rfx-backdrop|rfx-this-rfx-drawer/, "Carrier Fit should not use a modal drawer or backdrop");
assert.doesNotMatch(rfxEventsSource, /setThisRfxDrawerOpen|is-this-rfx-drawer-open/, "Carrier Fit should not retain drawer behavior");
...
assert.match(stylesSource, /\.rfx-carrier-fit-table-header,[\s\S]*?grid-template-columns:[\s\S]*?92px/, "Carrier Fit should use one full-width operational table with a dedicated selection column");
assert.match(rfxEventsSource, /alreadyInRfx[\s\S]*?Already in this RFx[\s\S]*?Not in RFx/, "Carrier Fit rows should show event membership inline and lock existing participants");
...
assert.match(rfxEventsSource, /const contactReadyCandidates = selectableCandidates\.filter\(\(\{ vendor \}\) => carrierCanReceiveOutreachChannel\(vendor, deliveryChannel\)\);/, "Carrier Fit should distinguish selectable matching carriers that can actually receive delivery");
...
assert.doesNotMatch(
  rfxEventsSource.slice(rfxEventsSource.indexOf("async function loadDetail"), rfxEventsSource.indexOf("function activateWorkbenchView")),
  /ensureSelectedEventChatThread\(/,
  "Loading Bid Room detail should not create an event thread"
);
assert.match(rfxEventsSource, /rfxChatStartEventThread\?\.addEventListener\("click"[\s\S]+ensureSelectedEventChatThread\(selectedEventId, \{ force: true, silent: true \}\)/, "Bid Room should create an event thread only from the explicit operator action");
...
assert.match(apiSource, /label: "Outreach lane hydration"[\s\S]+\.in\("vendor_id", vendorChunk\)[\s\S]+\.order\("id", \{ ascending: true \}\)[\s\S]+\.gt\("id", afterId\)/, "Outreach lane hydration should keyset-page only the requested carriers without skipping or duplicating rows");

...
assert.match(apiSource, /rateware_bid_room_follow_up_es_v2/, "MarkOS should provision the corrected Spanish Bid Room follow-up template");
assert.match(apiSource, /rateware_bid_room_follow_up_en_v2[\s\S]+language: "en_US"/, "MarkOS should provision the corrected English Bid Room follow-up with a Meta locale");
assert.match(apiSource, /stableTemplate[\s\S]+WHATSAPP_RFX_NOTIFICATION_PLACEHOLDERS[\s\S]+fallback_template/, "MarkOS should fall back to the approved RFx notifier while its dedicated follow-up template is unavailable");
assert.match(apiSource, /status: "sent",[\s\S]+delivery_status: "sent",[\s\S]+sent_at: now,[\s\S]+failed_at: null,[\s\S]+send_completed_at: now/, "A successful WhatsApp retry should clear its prior failure timestamp");
assert.match(apiSource, /metaSendWhatsappTemplate[\s\S]+status: "sent",[\s\S]+delivery_status: "sent",[\s\S]+sent_at: now,[\s\S]+failed_at: null,[\s\S]+provider: "meta"/, "Meta acceptance should clear stale WhatsApp failure state");
assert.match(apiSource, /rfx_events\(rfx_id,name,due_date\)[\s\S]+lane_count: "1"[\s\S]+due_date: cleanText\(event\.due_date\)/, "The approved MarkOS WhatsApp fallback should receive truthful RFx parameters");
assert.match(apiSource, /Responde o actualiza tu puja aqui: \{\{4\}\}[\s\S]+Si necesitas ayuda, responde a este mensaje\./, "WhatsApp follow-up templates must not end with a variable");
assert.match(apiSource, /providerErrorData\.details[\s\S]+provider\.error_user_msg|providerErrorData\.details[\s\S]+providerError\.error_user_msg/, "Meta WhatsApp errors should preserve actionable provider details");
...
assert.match(stylesSource, /\.bid-room-page \.shell-layout\.shell-focus-mode \{[\s\S]*?grid-template-columns: minmax\(0, 1fr\)/, "Bid Room focus mode should expand past the restored vertical navigation");
assert.match(stylesSource, /\.bid-room-page \.shell-focus-mode \.side-nav \{\s*display: none/, "Bid Room focus mode should hide its restored vertical navigation");
assert.match(stylesSource, /\.bid-room-page \.shell-layout\.shell-nav-collapsed \{[\s\S]*?grid-template-columns: 68px/, "Bid Room collapsed navigation should keep the shared narrow shell width");
assert.match(stylesSource, /\.bid-room-page \.shell-nav-collapsed \.side-nav a:not\(\.brand-mark\) \{[\s\S]*?font-size: 0/, "Bid Room collapsed navigation should hide module labels");
assert.match(stylesSource, /\.bid-room-page \.shell-layout\.shell-focus-mode[\s\S]*?display: block[\s\S]*?width: 100%/, "Bid Room focus mode should own the full canvas width over collapsed navigation");
assert.match(stylesSource, /\.bid-room-page \.shell-layout\.shell-focus-mode \.page-frame \{[\s\S]*?width: 100%/, "Bid Room focus mode page frame should fill the viewport");
...
assert.match(rfxBidApiSource, /XBF_BUY_SELL_DEFAULT_MARKUP_PCT = 15/, "Carrier portal API should target and default XBF buy-sell markup to 15 percent");
...
assert.match(rfxBidSource, /XBF_BUY_SELL_DEFAULT_MARKUP_PCT = 15/, "Carrier portal should explain the 15 percent XBF buy-sell target and default");
...
assert.match(rfxEventsSource, /XBF_BUY_SELL_DEFAULT_MARKUP_PCT = 15/, "Internal Bid Room should use the same 15 percent XBF buy-sell target and default markup");
...
assert.match(rfxBidApiSource, /markosSource[\s\S]*responseSource[\s\S]*markos_voice/, "A carrier-confirmed MarkOS decline should retain its voice source instead of masquerading as a portal click");
assert.match(rfxBidApiSource, /markos_session_id: markosSessionId/, "A MarkOS decline should retain the private call session in contact history");
...
assert.match(apiSource, /markos_conversations/, "Carrier relationship activity should include MarkOS voice conversations");
assert.match(apiSource, /markos_profile_updates/, "Carrier relationship activity should expose MarkOS profile proposals for review");
assert.match(vendorServiceSource, /review_markos_profile_update/, "Vendor service should submit an explicit MarkOS profile review decision");
assert.match(vendorsSource, /data-markos-review="approved"/, "Carrier CRM should let a reviewer approve MarkOS profile proposals");
assert.match(vendorsSource, /data-markos-review="rejected"/, "Carrier CRM should let a reviewer reject MarkOS profile proposals");
assert.match(vendorsSource, /data-markos-change-field/, "Carrier CRM should let a reviewer edit proposed values before approval");
assert.match(apiSource, /async function reviewMarkosProfileUpdate/, "Rateware API should own the MarkOS profile review workflow");
assert.match(rfxBidApiSource, /status: "profile_update_requested"[\s\S]+review_required: true[\s\S]+review_status: "pending"/, "MarkOS profile proposals should enter Carrier CRM as pending review instead of overwriting the profile");
assert.match(apiSource, /vendor\.markos_profile_update\.\$\{decision === "approved" \? "approve" : "reject"\}/, "MarkOS profile reviews should write an audit event");
assert.match(apiSource, /\.eq\("owner_email", user\.owner_email\)[\s\S]+\.eq\("status", "profile_update_requested"\)/, "MarkOS profile review should remain owner-scoped and request-scoped");
assert.match(apiSource, /function normalizeMarkosProfileChanges[\s\S]+rows\.slice\(0, 30\)/, "Carrier CRM review must preserve every profile change accepted by the MarkOS voice schema");
assert.match(rfxBidApiSource, /body\.action === "request_markos_followup"/, "MarkOS should delegate carrier-confirmed follow-up to Rateware");
assert.match(rfxBidApiSource, /async function markosBidRoomEmailRender/, "MarkOS Gmail follow-up should render from the official Bid Room HTML source");
assert.match(rfxBidApiSource, /cleanText\(metadata\.source\) !== "markos_voice"[\s\S]+marksman-email-signature\.png[\s\S]+<table/, "MarkOS Gmail should never recursively use a prior MarkOS follow-up as its Bid Room HTML source");
assert.match(rfxBidApiSource, /replace\(\/\^\(\?:Seguimiento\|Follow-up\)[\s\S]+sourceSubject/, "MarkOS Gmail should normalize an existing follow-up subject prefix before adding its own");
assert.match(rfxBidApiSource, /campaignSessionKey[\s\S]+MarkOS follow-up[^\n]+campaignSessionKey/, "Each MarkOS call session should receive its own outreach campaign so a later confirmed follow-up is not blocked as a duplicate");
assert.match(rfxBidApiSource, /email_template_source: "bid_room_official_html"/, "MarkOS Gmail follow-up should persist official Bid Room template provenance");
assert.match(rfxBidApiSource, /sourceHtml\.includes\("marksman-email-signature\.png"\)/, "MarkOS Gmail follow-up should require the branded MARKSMAN signature snapshot");
assert.match(rfxBidApiSource, /template_id: channel === "email" \? emailRender\?\.templateId/, "MarkOS Gmail follow-up should retain the Bid Room template id");
assert.doesNotMatch(rfxBidApiSource, /html_body: channel === "email" \? `<p>\$\{escapeHtmlText\(messageBody\)\}<\/p>`/, "MarkOS Gmail follow-up must not degrade to a plain paragraph");
assert.match(rfxBidApiSource, /status: "drafted"[\s\S]+review_required: false[\s\S]+review_status: "carrier_confirmed"/, "MarkOS follow-up should preserve an auditable draft while treating the carrier's spoken confirmation as send authorization");
assert.match(rfxBidApiSource, /dispatchMarkosConfirmedFollowUp\(messageIds, sessionId\)/, "MarkOS should dispatch the carrier-confirmed email and WhatsApp follow-up instead of stopping at a draft");
assert.match(rfxBidApiSource, /retryFailed[\s\S]+status === "drafted"[\s\S]+retryFailed && status === "failed"[\s\S]+manual_retry: retryFailed/, "MarkOS should expose an explicit manual retry for failed delivery without retrying sent or uncertain messages");
assert.match(rfxBidApiSource, /select\("id,metadata"\)[\s\S]+provider_state: providerState,[\s\S]+delivery,[\s\S]+provider_checked_at/, "A duplicate carrier receipt should refresh provider evidence without creating another history row");
assert.match(rfxBidApiSource, /body\.action === "confirm_markos_followup_receipt"/, "MarkOS should verify provider delivery and persist the carrier's live receipt response");
assert.match(rfxBidApiSource, /const historyStatus = receiptStatus === "received" \? "markos_followup_received" : "markos_followup_not_received"/, "Carrier-confirmed receipt should be written to the CRM timeline");
assert.match(rfxBidApiSource, /\.like\("status", "markos_call_%"\)/, "A follow-up receipt in the same session must not suppress final voice-conversation persistence");
assert.match(apiSource, /internalBody\.action === "send_markos_confirmed_followup"[\s\S]+isServiceRoleRequest\(request\)/, "The internal MarkOS sender should require server-to-server authorization before using connected providers");
assert.match(apiSource, /campaign_outcome_policy[\s\S]+followup_kind[\s\S]+outcome_support[\s\S]+!carrierConfirmed && !governedOutcomeSupport/, "Automatic outcome support should use its explicit campaign policy without pretending the carrier requested the message");
assert.match(apiSource, /sendOutreachMessages\(supabase, user[\s\S]+sendWhatsappOutreachMessages\(supabase, user/, "The internal MarkOS sender should reuse Rateware's guarded Gmail and WhatsApp delivery paths");
assert.match(rfxBidApiSource, /whatsapp_do_not_contact[\s\S]+whatsapp_opt_in_status[\s\S]+opted_out/, "MarkOS WhatsApp follow-up should honor the carrier suppression state");
assert.match(rfxBidApiSource, /markos_session_id: sessionId[\s\S]+followup_kind: followUpKind[\s\S]+carrier_confirmed: followUpKind === "requested_information"/, "MarkOS follow-up should distinguish carrier-requested information from automatic outcome support");
assert.match(rfxBidApiSource, /channels: \["email", "whatsapp"\][\s\S]+followup_kind: "outcome_support"/, "Every persisted MarkOS outcome should create bounded email and WhatsApp support");
assert.match(rfxBidApiSource, /addCarrierProfileUpdatePanel[\s\S]+Revisar y actualizar perfil/, "Every MarkOS support email should contain a visible Carrier CRM profile update request");
assert.match(rfxBidApiSource, /vendor_profile_requests[\s\S]+request_token_hash: await hashMarkosCarrierProfileToken/, "MarkOS should issue hashed private Carrier CRM links rather than expose stored tokens");
assert.match(rfxBidApiSource, /whatsapp_template_language: channel === "whatsapp" \? followUpLanguage : null/, "MarkOS WhatsApp follow-up should preserve the language selected during the call");
assert.match(apiSource, /if \(decision === "approved"\) \{[\s\S]+\.from\("vendors"\)[\s\S]+\.update\(patch\)/, "MarkOS should update the official carrier profile only on approval");

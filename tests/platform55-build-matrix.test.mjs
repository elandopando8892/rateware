...
const standaloneOspRoutes = new Set([
  "osp-preview.html",
  "provider-approvals.html",
  "provider-delivery.html",
  "provider-document-review.html",
  "provider-entity-vault.html",
  "provider-onboarding-app.html"
]);

assert.equal(internalRoutes.size, 22);
assert.equal(publicRoutes.size, 7);
assert.equal(expectedRoutes.size, 29);
assert.equal(standaloneOspRoutes.size, 6);

const trackedRootHtml = execFileSync("git", ["ls-files", "*.html"], { encoding: "utf8" })
  .split(/\r?\n/)
  .filter((entry) => entry && !entry.includes("/"))
  .sort();
assert.deepEqual(trackedRootHtml, [...new Set([...expectedRoutes, ...standaloneOspRoutes])].sort());

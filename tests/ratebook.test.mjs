...
assert.match(api, /const unresolvedCustomerKeys = new Set\(projects[\s\S]+businessNameKey\(project\.customer_name\)/, "Ratebook should identify legacy RFx projects that have a shipper name but no shipper id");
assert.match(api, /const uniqueShipperByName = new Map[\s\S]+uniqueShipperByName\.set\(key, null\)/, "Ratebook shipper fallback should reject ambiguous duplicate shipper names");
assert.match(api, /const shipperId = explicitShipperId \|\| cleanText\(shipper\.id\)/, "Ratebook should link an exact unique shipper-name match without overriding an explicit shipper id");

...
assert.equal(baseline.filter((entry) => entry.canonicalId.startsWith("edge.")).length, 305);
...
assert.equal(ACTION_CONTRACT.surfaces.filter((entry) => entry.decisionStatus === "pending_human_approval").length, 271);
...
assert.deepEqual({ total: finalBaseline.length, edge: finalBaseline.filter((entry) => entry.canonicalId.startsWith("edge.")).length, rpc: finalBaseline.filter((entry) => entry.canonicalId.startsWith("rpc.")).length }, { total: 417, edge: 305, rpc: 112 });

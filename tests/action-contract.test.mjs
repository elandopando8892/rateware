...
assert.equal(baseline.length, 419);
...
assert.equal(ACTION_CONTRACT.surfaces.length, 421);
...
assert.equal(ACTION_CONTRACT.surfaces.filter((entry) => entry.decisionStatus === "internal_only").length, 118);
...
assert.deepEqual({ total: finalBaseline.length, edge: finalBaseline.filter((entry) => entry.canonicalId.startsWith("edge.")).length, rpc: finalBaseline.filter((entry) => entry.canonicalId.startsWith("rpc.")).length }, { total: 419, edge: 304, rpc: 115 });

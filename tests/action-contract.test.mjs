...
assert.equal(baseline.length, 417);
assert.equal(baseline.filter((entry) => entry.canonicalId.startsWith("edge.")).length, 304);
assert.equal(baseline.filter((entry) => entry.canonicalId.startsWith("rpc.")).length, 113);
assert.equal(baseline.filter((entry) => entry.canonicalId.startsWith("edge.rateware-api.")).length, 255);
assert.equal(ACTION_CONTRACT.surfaces.length, 419);
assert.equal(ACTION_CONTRACT.surfaces.filter((entry) => entry.decisionStatus === "pending_human_approval").length, 270);
assert.equal(ACTION_CONTRACT.surfaces.filter((entry) => entry.decisionStatus === "explicitly_allowed").length, 33);
assert.equal(ACTION_CONTRACT.surfaces.filter((entry) => entry.decisionStatus === "internal_only").length, 116);
...
assert.deepEqual({ total: finalBaseline.length, edge: finalBaseline.filter((entry) => entry.canonicalId.startsWith("edge.")).length, rpc: finalBaseline.filter((entry) => entry.canonicalId.startsWith("rpc.")).length }, { total: 417, edge: 304, rpc: 113 });

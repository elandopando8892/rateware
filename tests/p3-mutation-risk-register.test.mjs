...
test("P3 register distinguishes durable delivery controls from open award risks", () => {
  ...
  assert.equal(byId.get("edge.rateware-api.create_rfx_award_package").control, "payload_bound_operation_ledger");
  assert.equal(byId.get("edge.rateware-api.mark_rfx_award_package_implementation_ready").control, "optimistic_version_guard");
  assert.equal(byId.get("edge.rateware-api.mark_rfx_award_package_implementation_ready").status, "controlled");
});

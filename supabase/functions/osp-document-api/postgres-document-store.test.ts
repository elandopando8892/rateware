...
Deno.test('Postgres document store performs profile review commands atomically under tenant authority', async () => {
  ...
  const sql = Object.assign(async (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    if (text.includes('promote_profile_review_complete_batch')) return [{ promotion_id: '66666666-6666-4666-8666-666666666666', promotion_status: 'applied', promoted_fact_count: 2, unchanged_fact_count: 1, withheld_field_count: 1, review_id: reviewId, review_revision: 4, replayed: false }];
    throw new Error(`UNEXPECTED_QUERY:${text}`);
  }, { begin: async <T>(operation: (tx: typeof sql) => Promise<T>) => await operation(sql) });
  const store = createPostgresDocumentStore({ databaseUrl: 'postgres://localhost:55322/osp', postgresFactory: () => sql });
  ...
  assertEquals(await store.promoteProfileReviewFacts({ organizationId, reviewId, expectedRevision: 4, candidateSha256: sourceSha256, comparisonSha256: 'b'.repeat(64), expectedCurrentFactIds: { legal_name: null }, actorSubject: 'ops-subject', actorPermission: 'osp:operate' }), {
  ...
  assertEquals(queries.some((query) => query.text.includes('promote_profile_review_complete_batch') && query.values.includes(sourceSha256)), true);
  assertEquals(queries.some((query) => query.text.includes('promote_profile_review_complete_batch') && query.text.includes('::text::jsonb') && query.values.includes('b'.repeat(64))), true);

...
Deno.test('Postgres document store approves only the reviewed persisted source and supersedes the prior approval', async () => {
  ...
  const sql = (async (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    if (text.includes('from osp_private.document_versions version') && text.includes('for update')) return [{
      ...
      status: 'review_required', source_sha256: sourceSha256, aggregate_version: '1',
    }];
  ...
  const mismatchSql = Object.assign(async (strings: TemplateStringsArray) => {
      ...
      if (text.includes('for update')) return [{ id: '33333333-3333-4333-8333-333333333333', document_id: '22222222-2222-4222-8222-222222222222', version: 1, status: 'review_required', source_sha256: 'b'.repeat(64), aggregate_version: 1 }];
      return [];

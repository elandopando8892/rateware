  ...
  try {
    ...
    const outcome = await deliverClaimedNotification({
      ...
      send: async () => {
        ...
        const replyHref = `mailto:${encodeURIComponent(lead.email)}?subject=${encodeURIComponent(`Seguimiento de tarifario · folio ${row.id}`)}`;
        const plain = ["Nuevo tarifario de transportista", ...entries.map(([label, value]) => `${label}: ${value}`), "", "Acción comercial:", ...nextSteps.map((step, index) => `${index + 1}. ${step}`), "", "Preparar respuesta al transportista:", replyHref, "", "Descarga privada (7 días):", signedUrl.data.signedUrl].join("\n");
        const html = `<h2>Nuevo tarifario de transportista</h2><table>${entries.map(([label, value]) => `<tr><th align="left">${escapeHtml(label)}</th><td>${escapeHtml(value)}</td></tr>`).join("")}</table><h3>Acción comercial</h3><ol>${nextSteps.map(step => `<li>${escapeHtml(step)}</li>`).join("")}</ol><p><a href="${escapeHtml(replyHref)}">Preparar respuesta al transportista</a></p><p><a href="${escapeHtml(signedUrl.data.signedUrl)}">Descargar archivo original (7 días)</a></p>`;
        const response = await fetch("https://gmail.googleapis.com/gmail/v1/users/me/messages/send", {

import { ClientError, conflict, forbidden } from './http-error.ts';
...
function revision(value:unknown){
  ...
  if(!Number.isInteger(number)||number<1) throw new ClientError('expected_revision must be a positive integer.');
  return number;
}
...
async function ownedReview(supabase:any,organizationId:string,reviewId:string,reviewerUserId:string,expectedRevision:number){
  ...
  if(result.error) throw result.error;
  if(!result.data) throw conflict('Review ownership or revision conflict.');
...
export async function claimProviderEntityDocumentReview(supabase:any,input:Record<string,unknown>,reviewerUserId:string){
  ...
  if(!claimed.data) throw conflict('Review is not claimable or its revision changed.');
...
export async function decideProviderEntityReviewField(supabase:any,input:Record<string,unknown>,reviewerUserId:string){
  ...
  if(!FIELD_DECISIONS.has(decision)) throw new ClientError('Unsupported field decision.');
  if(decision==='corrected'&&(input.reviewer_value===null||input.reviewer_value===undefined)) throw new ClientError('reviewer_value is required for a correction.');
  ...
  if(!field.data) throw conflict('Review field is not pending.');
  if(!FIELD_CODE.test(String(field.data.field_code||''))) throw new Error('Review field code is invalid.');
  if(decision==='withheld'&&!['restricted','highly_restricted'].includes(field.data.sensitivity)) throw new ClientError('Only restricted fields may be withheld.');
  ...
  if(!guard.data) throw conflict('Review revision changed.');
  ...
  if(!decided.data) throw conflict('Review field changed before the decision was stored.');
...
export async function finalizeProviderEntityDocumentReview(supabase:any,input:Record<string,unknown>,reviewerUserId:string){
  ...
  const decision=required(input.decision,'decision');
  const note=required(input.decision_note,'decision_note');
  if(!REVIEW_DECISIONS.has(decision)) throw new ClientError('Unsupported review decision.');
  const row=await ownedReview(supabase,organizationId,reviewId,actor,expectedRevision);
  if(row.requested_by_user_id&&row.requested_by_user_id===actor) throw forbidden('Requester cannot finalize their own review.');
  ...
  if(statuses.includes('pending')) throw conflict('All review fields must be decided first.');
  if(decision==='approved'&&statuses.includes('rejected')) throw conflict('A review with rejected fields cannot be approved.');
  ...
  if(!finalized.data) throw conflict('Review revision changed.');

...
function uuid(value:unknown,field:string){
  ...
  if(!UUID.test(result)) throw new Error(`${field} must be a valid UUID.`);
  return result;
}
...
export async function recordProviderOnboardingRequirementWaiver(
  ...
  if(!requirement.data) throw new Error('Requirement was not found.');
  if(!requirement.data.active) throw new Error('Requirement is not active; there is nothing to waive.');

  const authorizedAt=new Date().toISOString();
  const validated=validateWaiverRequest({
  ...
  if(inserted.error){
    if(String(inserted.error.code)==='23505'){
      throw new Error('An active waiver already exists for this requirement; revoke it first.');
    }
...
export async function revokeProviderOnboardingRequirementWaiver(
  ...
  if(reason.length<10) throw new Error('revocation_reason must explain why the override no longer stands.');
  if(reason.length>2000) throw new Error('revocation_reason is too long.');
  ...
  if(!revoked.data) throw new Error('Active waiver was not found.');

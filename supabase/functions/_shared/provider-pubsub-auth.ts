...
async function loadGoogleJwks(force = false) {
  if (!force && cachedJwks && cachedJwks.expiresAt > Date.now()) return cachedJwks.keys;
  const response = await fetch(GOOGLE_JWKS_URL, { headers: { Accept: 'application/json' } });
  if (!response.ok) throw new Error('Google OIDC signing keys could not be loaded.');
  ...
  cachedJwks = { keys, expiresAt: Date.now() + Math.max(300, Math.min(maxAge, 3600)) * 1000 };
  return keys;
}

async function verifySignature(token: string, header: Record<string, unknown>) {
  if (header.alg !== 'RS256') throw new Error('Pub/Sub OIDC token must use RS256.');
...
export async function verifyProviderPubSubRequest(request: Request) {
  const expectedAudience = clean(Deno.env.get('PROVIDER_GMAIL_PUBSUB_AUDIENCE'));
  const expectedServiceAccount = clean(Deno.env.get('PROVIDER_GMAIL_PUBSUB_SERVICE_ACCOUNT'))?.toLowerCase();
  ...
  if (!token) throw new Error('Pub/Sub push request is missing a Bearer token.');
  const segments = token.split('.');
  if (segments.length !== 3 || segments.some((segment) => !segment)) throw new Error('Pub/Sub OIDC token is malformed.');
  ...
  if (!GOOGLE_ISSUERS.has(String(claims.iss || ''))) throw new Error('Pub/Sub OIDC issuer is invalid.');
  if (!audienceMatches(claims.aud, expectedAudience)) throw new Error('Pub/Sub OIDC audience is invalid.');
  if (!Number.isFinite(exp) || exp < now - 30) throw new Error('Pub/Sub OIDC token is expired.');
  if (!Number.isFinite(iat) || iat > now + 300) throw new Error('Pub/Sub OIDC issued-at claim is invalid.');
  const email = clean(claims.email)?.toLowerCase();
  if (email !== expectedServiceAccount) throw new Error('Pub/Sub OIDC service account is invalid.');
  if (!(claims.email_verified === true || claims.email_verified === 'true')) throw new Error('Pub/Sub OIDC service account email is not verified.');

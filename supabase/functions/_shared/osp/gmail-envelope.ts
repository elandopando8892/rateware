...
export async function parseCopiedRequest(rawMime: Uint8Array): Promise<ParsedCopiedRequest> {
  ...
  const from = header(raw, 'from'); const to = header(raw, 'to'); const subject = header(raw, 'subject') ?? '';
  ...
  const recipients = [...toMailboxes, ...parsedCc];
  const supplierAddress = recipients.find((address) => !['xbfreight.com', 'heymarksman.com'].includes(address.split('@')[1] ?? ''));
  const supplierDomain = supplierAddress?.split('@')[1] ?? '';
  const approvedSender = senderDomain === 'xbfreight.com' || sender === 'sales@heymarksman.com';
  if (!sender || !to || !approvedSender || !supplierDomain || !recipients.includes('carriers@xbfreight.com')) throw new Error('UNQUALIFIED_GMAIL_MESSAGE');

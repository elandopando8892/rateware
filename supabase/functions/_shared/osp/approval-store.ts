      ...
      try {
        return await withOrganizationTransaction(
          ...
          async (tx) => {
            ...
            if (command.type === "complete_operations_review") {
              rows =
                await tx`select * from osp_private.complete_operations_review_command(${command.organizationId}, ${command.caseId}, ${command.inputSnapshotSha256}, ${command.expectedCaseVersion}, ${command.actor.subject}, ${command.actor.verifiedEmail}, ${permissions}, ${command.actor.role}, ${command.actor.authorizationSessionId}, ${command.actor.authorizationSessionIssuedAt}, ${hash})`;
            } else if (command.type === "approve_signature") {
              rows =
                await tx`select * from osp_private.approve_signature_command(${command.organizationId}, ${command.caseId}, ${command.inputSnapshotSha256}, ${command.signatureVaultRef}, ${command.signaturePositionVersion}, ${command.expectedCaseVersion}, ${command.idempotencyKey}, ${command.actor.subject}, ${command.actor.verifiedEmail}, ${permissions}, ${command.actor.role}, ${command.actor.authorizationSessionId}, ${command.actor.authorizationSessionIssuedAt}, ${hash})`;
            } else if (command.type === "authorize_outbound") {
              rows =
                await tx`select * from osp_private.authorize_outbound_command(${command.organizationId}, ${command.caseId}, ${command.payloadId}, ${command.payloadSha256}, ${command.attachmentSha256}, ${command.expectedCaseVersion}, ${command.idempotencyKey}, ${command.actor.subject}, ${command.actor.verifiedEmail}, ${permissions}, ${command.actor.role}, ${command.actor.authorizationSessionId}, ${command.actor.authorizationSessionIssuedAt}, ${hash})`;
            } else {
              fail("APPROVAL_TRANSITION_INVALID");
            }

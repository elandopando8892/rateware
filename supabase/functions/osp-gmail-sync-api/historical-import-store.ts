...
function databaseUrl(value: string): string {
  try {
    ...
    if (
      ...
      url.hash ||
      (url.search && !/^\?sslmode=(?:require|prefer)$/.test(url.search))
    ) throw new Error();
...
function exactRow(rows: Record<string, unknown>[]): HistoricalImportClaim {
  ...
  if (
    ...
    !Number.isSafeInteger(attachmentMetadataRows) ||
    attachmentMetadataRows < 0 || attachmentMetadataRows > 100 ||
...
export function createPostgresHistoricalImportStore(options: {
  ...
  return Object.freeze({
    async record(
      input: HistoricalImportClaimInput,
    ): Promise<HistoricalImportClaim> {
      return await withOrganizationTransaction(
        sql,
        input.organizationId,
        async (tx) =>
          exactRow(
            await tx`with receipt as (select * from osp_private.record_historical_gmail_import(
            ${input.organizationId},
            ${input.mailboxEmail},

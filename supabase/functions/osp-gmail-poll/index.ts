...
function databaseConnection(value: string): string {
  ...
  const allowedQuery = parsed.searchParams.size === 1 && ['require', 'prefer'].includes(sslMode ?? '');
  if (!['postgres:', 'postgresql:'].includes(parsed.protocol) || !parsed.hostname || (parsed.search && !allowedQuery) || parsed.hash) {
    throw new Error('INVALID_RUNTIME_CONFIGURATION');
  }
...
try {
  ...
  if (expectedToken.length < 32) throw new Error('INVALID_RUNTIME_CONFIGURATION');
  const sql = postgres(databaseConnection(required('SUPABASE_DB_URL')), { max: 1, prepare: false });
  const supabase = createClient(supabaseUrl, serviceRoleKey, {
    auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
  });
  const organizationId = OSP_PRODUCTION_ORGANIZATION_BINDING.canonicalOrganization;
  ...
  runtime = createScheduledGmailPollHandler({
    ...
    claim: async () => {
      ...
      `;
      if (rows.length === 1) return { status: 'claimed' as const, leaseId: String(rows[0].gmail_poll_lease_id) };
      ...
      `;
      return { status: control[0]?.gmail_poll_enabled === true ? 'busy' as const : 'disabled' as const };
    },
    poll: async () => {
    ...
    complete: async (receipt: ScheduledGmailPollReceipt, leaseId: string) => {
      await sql`
        ...
        set gmail_poll_last_completed_at = statement_timestamp(),
            ...
            gmail_poll_last_receipt = ${sql.json({
  ...
  console.error('OSP_GMAIL_POLL_BOOT_FAILED', error instanceof Error ? error.message : 'UNKNOWN_BOOT_ERROR');
  runtime = () => Promise.resolve(new Response(JSON.stringify({ error: { code: 'INTERNAL_ERROR', incident_id: crypto.randomUUID() } }), {
    status: 500,

...
Deno.test('XLSX adapter preserves typed cells, inert formulas, merges, and coordinates', async () => {
  ...
  assertEquals(parsed.sheets[0].mergedRanges, ['D4:E4']);
  assertEquals(parsed.sheets[0].cells.map((cell) => ({ address: cell.address, kind: cell.kind, value: cell.value })), [
  ...
  assertEquals(parsed.evidence.every((item) => item.locator.sourceVersionId === sourceVersionId), true);
  assertEquals(parsed.evidence.find((item) => item.locator.cellRange === 'C3')?.content, 'FORMULA:B2*2');
});

Deno.test('XLSX adapter rejects corrupt and oversized sources', async () => {
  await assertRejects(() => parseXlsxStructure({ sourceVersionId, bytes: new Uint8Array([1, 2, 3]) }), Error, 'XLSX_INVALID');
  await assertRejects(() => parseXlsxStructure({ sourceVersionId, bytes: new Uint8Array(25 * 1024 * 1024 + 1) }), Error, 'DOCUMENT_SIZE_INVALID');
});

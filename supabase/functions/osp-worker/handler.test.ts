...
Deno.test("OpenAI exact-case preflight is service-only and never drains jobs", async () => {
  let calls = 0;
  const handler = createOspWorkerHandler({
    expectedToken: token,
    manualCanaryToken: manualToken,
    enqueue: () => Promise.reject(new Error("GLOBAL_QUEUE_CALLED")),
    run: () => Promise.reject(new Error("GLOBAL_QUEUE_CALLED")),
    preflightOpenAiExactCase: async () => {
      calls += 1;
      return { httpStatus: 200, parseCode: null };
    },
  });
  const body = { action: "preflight_openai_exact_case" };
  assertEquals(
    (await handler(request(body, `Bearer ${manualToken}`))).status,
    401,
  );
  ...
  const response = await handler(request(body));
  assertEquals(response.status, 200);
  assertEquals(await response.json(), { httpStatus: 200, parseCode: null });
  assertEquals(calls, 1);
});

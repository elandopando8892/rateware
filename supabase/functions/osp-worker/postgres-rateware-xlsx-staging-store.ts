...
import type {
  RatewareXlsxStageReceipt,
  RatewareXlsxStagingStore,
} from "./rateware-xlsx-staging.ts";
...
function receipt(row: SqlRow): RatewareXlsxStageReceipt {
  if (
...
export function createPostgresRatewareXlsxStagingStore(options: {
  databaseUrl: string;
  ...
  const created =
    (options.postgresFactory ?? postgres as unknown as PostgresFactory)(
      requireDatabaseUrl(options.databaseUrl),
      {
        ...
        connect_timeout: 5,
        connection: {
          application_name: "osp-rateware-xlsx-routing",
  ...
  const store: RatewareXlsxStagingStore = {
    async stage(input) {
      return await withWorkerTransaction(sql, async (tx) => {
        const rows =
          await tx`select raw_upload_id, interpretation_job_id, rate_staging_id, inserted from osp_private.stage_rateware_xlsx_quote_from_lease(${input.organizationId}, ${input.caseId}, ${input.jobId}, ${input.leaseToken}, ${input.documentVersionId}, ${input.sourceSha256}, ${

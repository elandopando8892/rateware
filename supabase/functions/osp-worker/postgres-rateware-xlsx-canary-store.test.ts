...
Deno.test("Rateware XLSX canary store stages only through the worker function", async () => {
  const calls: string[] = [];
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      calls.push(strings.join("?") + JSON.stringify(values));
      if (/stage_rateware_xlsx_quote/.test(strings.join("?"))) {
        return [{
  ...
  const store = createPostgresRatewareXlsxCanaryStore({
  ...
  assertEquals(
    calls.some((call) => /stage_rateware_xlsx_quote/.test(call)),
    true,
  );
});

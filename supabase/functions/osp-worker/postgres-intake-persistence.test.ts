...
Deno.test('Postgres intake persistence replays a delivery receipt without creating a second case', async () => {
  ...
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (/insert into osp_private\.gmail_messages/.test(query)) {
        messageInsertValues.push(values);
        return [];
      }
      if (
        /insert into osp_private\.gmail_attachments|insert into osp_private\.case_events/
          .test(query)
      ) return [];
      if (
        /select id, aggregate_version from osp_private\.customer_registration_cases/
          .test(query)
      ) return [{ id: values[1], aggregate_version: 0 }];
      ...
      if (/insert into osp_private\.command_receipts/.test(query)) {
        receipts.set(`${values[2]}:${values[3]}`, {
  ...
  const replay = await persistence.createCase(
    {
      ...input,
      source: {
        ...input.source,
...
Deno.test('Postgres exact attachment does not insert a second gmail row for the same source', async () => {
  ...
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (
        /update osp_private\.gmail_messages set duplicate_evidence_json/.test(
          query,
        )
      ) return [];
      if (
  ...
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (
        ...
        evidenceUpdates += 1;
        evidenceUpdateValues.push(values);
        return [];
      }
      if (
        /select id, aggregate_version from osp_private\.customer_registration_cases/
          .test(query)
      ) return [{ id: caseId, aggregate_version: 0 }];
      if (/select coalesce\(max\(sequence\)/.test(query)) {
        return [{ sequence: 1 }];
      }
      if (
      ...
      if (/insert into osp_private\.case_events/.test(query)) {
...
Deno.test('Postgres probable hold appends evidence and advances the case aggregate', async () => {
  ...
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (
        /insert into osp_private\.gmail_messages|insert into osp_private\.gmail_attachments|insert into osp_private\.duplicate_candidates/
          .test(query)
      ) return [];
      if (
        /select id(?:, aggregate_version)? from osp_private\.customer_registration_cases/
...
Deno.test('Postgres duplicate review refresh appends evidence once and advances the aggregate on replay', async () => {
  ...
  const sql = Object.assign(
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (/insert into osp_private\.duplicate_candidates/.test(query)) {
        return [];
      }
      if (
        /select id, aggregate_version from osp_private\.customer_registration_cases/
          .test(query)
      ) return [{ id: caseId, aggregate_version: 4 }];
      if (/select coalesce\(max\(sequence\)/.test(query)) {
        return [{
          sequence: 2,
        }];
      ...
      if (
        /update osp_private\.customer_registration_cases set blocked_by_duplicate_review = true, aggregate_version = aggregate_version \+ 1/
          .test(query)
      ) {
        aggregateUpdates += 1;

...
export type PostgresIntakePersistenceOptions = { databaseUrl: string; postgresFactory?: PostgresFactory };

function requireDatabaseUrl(value: string): string {
  try {
    const url = new URL(value);
    if (value.trim() !== value || !['postgres:', 'postgresql:'].includes(url.protocol) || !url.hostname || url.search || url.hash) throw new Error('INVALID_RUNTIME_CONFIGURATION');
    return value;
...
function candidate(row: SqlRow): DuplicateCandidate {
  if (typeof row.case_id !== 'string' || typeof row.gmail_message_id !== 'string' || typeof row.raw_mime_hash !== 'string' ||
      typeof row.gmail_thread_id !== 'string' || typeof row.supplier_domain !== 'string' || typeof row.received_at !== 'string') {
...
function evidenceFor(candidateCaseId: string, evidence: readonly DuplicateSignal[]): string {
  return JSON.stringify(evidence.filter((item) => item.sourceIds.includes(candidateCaseId)));
}
...
function stableSource(source: IntakeSource): Record<string, unknown> {
  return {
    ...
    attachments: source.attachments.map(({ sha256, contentType }) => ({ sha256, contentType })).sort((left, right) => canonical(left).localeCompare(canonical(right))),
...
function receiptResponse(value: unknown): Record<string, unknown> {
  if (typeof value === 'string') {
    try { value = JSON.parse(value); } catch { throw new Error('DATABASE_TEMPORARY'); }
  }
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('DATABASE_TEMPORARY');
  return value as Record<string, unknown>;
}

function createdResponse(value: unknown): { caseId: string; eventId: string } {
  const response = receiptResponse(value);
  if (typeof response.caseId !== 'string' || typeof response.eventId !== 'string') throw new Error('DATABASE_TEMPORARY');
  return { caseId: response.caseId, eventId: response.eventId };
}

function heldResponse(value: unknown): { caseId: string } {
  const response = receiptResponse(value);
  if (typeof response.caseId !== 'string') throw new Error('DATABASE_TEMPORARY');
  return { caseId: response.caseId };
}

function refreshResponse(value: unknown): { caseId: string; eventId: string | null } {
  const response = receiptResponse(value);
  if (typeof response.caseId !== 'string' || (response.eventId !== null && typeof response.eventId !== 'string')) throw new Error('DATABASE_TEMPORARY');
  return { caseId: response.caseId, eventId: response.eventId as string | null };
}

async function withReceipt<T>(tx: SqlPort, organizationId: string, operation: string, idempotencyKey: string, request: unknown, parse: (value: unknown) => T, effect: () => Promise<T>): Promise<T> {
  if (!/^[A-Za-z0-9:_-]{1,256}$/.test(idempotencyKey)) throw new Error('INVALID_DELIVERY_KEY');
  ...
  await tx`insert into osp_private.command_receipts (id, organization_id, operation, idempotency_key, request_hash, response_json) values (${crypto.randomUUID()}, ${organizationId}, ${operation}, ${idempotencyKey}, ${hash}, ${JSON.stringify(response)})`;
  return response;
}

async function persistSource(tx: SqlPort, organizationId: string, caseId: string, source: IntakeSource, parsed: ParsedCopiedRequest, evidence: readonly DuplicateSignal[] = []): Promise<void> {
  const messageId = crypto.randomUUID();
  const rawMimeKey = requireObjectKey(source.rawMimeKey);
  const rawMimeHash = requireHash(source.rawMimeHash);
  if (!Number.isFinite(Date.parse(source.receivedAt))) throw new Error('INVALID_SOURCE_OBJECT');
  ...
  if (existing.length === 1) {
    ...
    if (typeof row.id !== 'string' || row.case_id !== caseId || row.source_sha256 !== rawMimeHash || row.gmail_thread_id !== source.gmailThreadId) throw new Error('IDEMPOTENCY_CONFLICT');
    if (evidence.length > 0) await tx`update osp_private.gmail_messages set duplicate_evidence_json = ${JSON.stringify(evidence)} where organization_id = ${organizationId} and id = ${row.id}`;
    return;
  }
  await tx`insert into osp_private.gmail_messages (id, organization_id, gmail_message_id, gmail_thread_id, case_id, opaque_object_key, source_sha256, sender_domain, subject, to_addresses, cc_addresses, safe_body, application_reference, requirement_tokens, duplicate_evidence_json, received_at) values (${messageId}, ${organizationId}, ${source.gmailMessageId}, ${source.gmailThreadId}, ${caseId}, ${rawMimeKey}, ${rawMimeHash}, ${parsed.senderDomain}, ${parsed.subject}, ${parsed.to}, ${parsed.cc}, ${parsed.safeBody}, ${parsed.applicationReference}, ${parsed.requirementTokens}, ${JSON.stringify(evidence)}, ${source.receivedAt})`;
...
async function persistCandidates(tx: SqlPort, organizationId: string, caseId: string, candidateIds: readonly string[], evidence: readonly DuplicateSignal[]): Promise<void> {
  for (const candidateCaseId of [...new Set(candidateIds)].sort()) {
    ...
    const score = Math.min(1, related.reduce((total, item) => total + item.score, 0));
...
async function appendEvent(tx: SqlPort, organizationId: string, caseId: string, reason: string, correlationId: string, evidence: readonly DuplicateSignal[] = []): Promise<string> {
  ...
  if (locked.length !== 1 || (typeof locked[0].aggregate_version !== 'number' && typeof locked[0].aggregate_version !== 'string')) throw new Error('DATABASE_TEMPORARY');
  const sourceVersion = Number(locked[0].aggregate_version);
  if (!Number.isSafeInteger(sourceVersion) || sourceVersion < 0) throw new Error('DATABASE_TEMPORARY');
  ...
  if (!Number.isSafeInteger(sequence) || sequence < 1) throw new Error('DATABASE_TEMPORARY');
  ...
  await tx`insert into osp_private.case_events (id, organization_id, case_id, sequence, state, actor_subject, authority_role, source_version, occurred_at, reason_code, correlation_id, evidence_json) values (${eventId}, ${organizationId}, ${caseId}, ${sequence}, 'received', 'osp-worker', 'workflow', ${sourceVersion}, now(), ${reason}, ${correlationId}, ${JSON.stringify(evidence)})`;
  return eventId;
}

async function appendReviewEvent(tx: SqlPort, organizationId: string, caseId: string, correlationId: string, evidence: readonly DuplicateSignal[]): Promise<string> {
  const locked = await tx`select id, aggregate_version from osp_private.customer_registration_cases where organization_id = ${organizationId} and id = ${caseId} for update`;
  if (locked.length !== 1 || typeof locked[0].aggregate_version !== 'number' && typeof locked[0].aggregate_version !== 'string') throw new Error('DATABASE_TEMPORARY');
  const sourceVersion = Number(locked[0].aggregate_version);
  if (!Number.isSafeInteger(sourceVersion) || sourceVersion < 0) throw new Error('DATABASE_TEMPORARY');
  ...
  if (updated.length !== 1) throw new Error('VERSION_CONFLICT');
  const eventId = crypto.randomUUID();
  await tx`insert into osp_private.case_events (id, organization_id, case_id, sequence, state, actor_subject, authority_role, source_version, occurred_at, reason_code, correlation_id, evidence_json) values (${eventId}, ${organizationId}, ${caseId}, ${sequence}, 'received', 'osp-worker', 'workflow', ${sourceVersion}, now(), 'duplicate_review_refresh', ${correlationId}, ${JSON.stringify(evidence)})`;
  return eventId;
}

async function queryDuplicates(tx: SqlPort, organizationId: string): Promise<readonly DuplicateCandidate[]> {
...
export function createPostgresIntakePersistence(options: PostgresIntakePersistenceOptions): IntakePersistence {
  const created = (options.postgresFactory ?? postgres as unknown as PostgresFactory)(requireDatabaseUrl(options.databaseUrl), {
    ssl: 'verify-full', fetch_types: false, prepare: false, max: 1, connect_timeout: 5,
    connection: { application_name: 'osp-gmail-intake', statement_timeout: '3000' },
  });
  if (typeof created !== 'function') throw new Error('INVALID_RUNTIME_CONFIGURATION');
  const sql = created as SqlPort;
  const findDuplicates = async (organizationId: string, _source: DuplicateCandidate): Promise<readonly DuplicateCandidate[]> => await withOrganizationTransaction(sql, organizationId, async (tx) => await queryDuplicates(tx, organizationId));
  return Object.freeze({
    findDuplicates,
    async createCase(input) {
      return await withOrganizationTransaction(sql, input.organizationId, async (tx) => {
        const request = { source: stableSource(input.source), parsed: stableParsed(input.parsed) };
        return await withReceipt(tx, input.organizationId, 'gmail_intake_create', input.deliveryIdempotencyKey, request, createdResponse, async () => {
          ...
          if (supplierRows.length !== 1 || typeof supplierRows[0].id !== 'string') throw new Error('DATABASE_TEMPORARY');
          ...
          await persistSource(tx, input.organizationId, caseId, input.source, input.parsed);
          const eventId = await appendEvent(tx, input.organizationId, caseId, 'gmail_intake_created', input.deliveryIdempotencyKey);
    ...
    async attachExact(input) {
      return await withOrganizationTransaction(sql, input.organizationId, async (tx) => {
        const request = { existingCaseId: input.existingCaseId, source: stableSource(input.source), parsed: stableParsed(input.parsed), evidence: input.evidence };
        return await withReceipt(tx, input.organizationId, 'gmail_intake_attach_exact', input.deliveryIdempotencyKey, request, createdResponse, async () => {
          await persistSource(tx, input.organizationId, input.existingCaseId, input.source, input.parsed, input.evidence);
          const eventId = await appendEvent(tx, input.organizationId, input.existingCaseId, 'gmail_intake_attached_exact', input.deliveryIdempotencyKey, input.evidence);
    ...
    async holdForReview(input) {
      return await withOrganizationTransaction(sql, input.organizationId, async (tx) => {
        const request = { source: stableSource(input.source), parsed: stableParsed(input.parsed), candidateIds: input.candidateIds, evidence: input.evidence };
        return await withReceipt(tx, input.organizationId, 'gmail_intake_hold_duplicate_review', input.deliveryIdempotencyKey, request, heldResponse, async () => {
          const supplierRows = await tx`insert into osp_private.supplier_counterparties (id, organization_id, legal_name) values (${crypto.randomUUID()}, ${input.organizationId}, ${input.parsed.supplierDomain}) on conflict (organization_id, legal_name) do update set legal_name = excluded.legal_name returning id`;
          if (supplierRows.length !== 1 || typeof supplierRows[0].id !== 'string') throw new Error('DATABASE_TEMPORARY');
          const caseId = crypto.randomUUID();
          ...
          await persistSource(tx, input.organizationId, caseId, input.source, input.parsed, input.evidence);
          await persistCandidates(tx, input.organizationId, caseId, input.candidateIds, input.evidence);
          await appendEvent(tx, input.organizationId, caseId, 'duplicate_review_required', input.deliveryIdempotencyKey, input.evidence);
          return { caseId };
        });
      });
    },
    async refreshDuplicateReview(input) {
      await withOrganizationTransaction(sql, input.organizationId, async (tx) => await withReceipt(tx, input.organizationId, 'duplicate_review_refresh', input.correlationId, { caseId: input.caseId }, refreshResponse, async () => {
        ...
        const assessment = assessDuplicates(current, all.filter((item) => item.caseId !== input.caseId));
        if (assessment.outcome !== 'probable') return { caseId: input.caseId, eventId: null };
        await persistCandidates(tx, input.organizationId, input.caseId, assessment.candidateCaseIds, assessment.evidence);
        const eventId = await appendReviewEvent(tx, input.organizationId, input.caseId, input.correlationId, assessment.evidence);

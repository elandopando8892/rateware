  ...
  const source: CurrentOutboundAuthorizationSource = {
    resolveCurrent: async (
      ...
      await withOrganizationTransaction(
        ...
        async (tx) => {
          const rows =
            await tx`select payload.organization_id, payload.case_id, payload.id, payload.payload_kind, payload.case_version, payload.canonical_sha256, payload.attachment_sha256s from osp_private.outbound_payloads payload join osp_private.customer_registration_cases case_record on case_record.organization_id = payload.organization_id and case_record.id = payload.case_id and case_record.aggregate_version = payload.case_version where payload.organization_id = ${input.organizationId} and payload.case_id = ${input.caseId} and payload.id = ${input.payloadId} and payload.status = 'frozen'`;
          if (rows.length !== 1) {
            throw new Error("OUTBOUND_AUTHORIZATION_STALE");
          }
          const row = rows[0];

...
function base64Data(value: unknown) {
  ...
  const normalized = text.replace(/-/g, "+").replace(/_/g, "/").padEnd(
...
function parseNotification(payload: Record<string, any>) {
  ...
  if (!message || typeof message !== "object") {
    throw new Error("Pub/Sub envelope is missing message.");
  }
  const pubsubMessageId = cleanProviderGmailText(
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      throw new Error("Gmail push notification data is not valid JSON.");
    }
    throw error;
  }
  const emailAddress = cleanProviderGmailText(decoded.emailAddress)
    ?.toLowerCase();
  const historyId = cleanProviderGmailText(decoded.historyId);
  if (!emailAddress || !historyId || !/^\d+$/.test(historyId)) {
    throw new Error(
      "Gmail push notification must include emailAddress and numeric historyId.",
    );
  }
  return {
    pubsubMessageId,
...
async function triggerOspWorker() {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
...
Deno.serve(async (request) => {
  if (request.method !== "POST") {
    return response(405, { error: "POST is required." });
  }
  ...
  if (notification.emailAddress !== allowedMailbox) {
    return response(400, {
      error: "Gmail notification mailbox is not allowed.",
    });
  }
  ...
  try {
    ...
    if (rows.length !== 1) {
      return response(503, {
        error: "Provider Gmail push routing is ambiguous for this mailbox.",
      });
    }
    ...
    if (!eventResult.data) {
      throw new Error("Provider Gmail push event could not be persisted.");
    }
    eventId = eventResult.data.id;
    if (["completed", "ignored_stale"].includes(eventResult.data.status)) {
      return response(204);
    }

    if (historyAtOrBefore(notification.historyId, connection.history_id)) {
      await triggerOspWorker();
    ...
    if (!claimed.data) {
      return response(503, {
        error: "Provider Gmail push event is already processing.",
      });
    }

create table osp_private.outbound_preprovider_reconciliations (
  ...
  resolution_code text not null
    check (resolution_code = 'GMAIL_MIME_TENANT_CONTEXT_MISSING'),
...
create trigger outbound_preprovider_reconciliations_append_only
before update or delete on osp_private.outbound_preprovider_reconciliations
for each row execute function osp_private.reject_approval_mutation();

create function osp_private.prepare_reconciled_outbound_retry(
...
as $$
declare
  resolution constant text := 'GMAIL_MIME_TENANT_CONTEXT_MISSING';
  ...
  prior osp_private.outbound_preprovider_reconciliations%rowtype;
  created_attempt_id uuid := extensions.gen_random_uuid();
  created_job_id uuid := extensions.gen_random_uuid();
  created_idempotency_key text := 'reconciled:' || p_attempt_id::text || ':mime-scope-v1';
  next_version bigint;
begin
  if p_gmail_evidence_sha256 !~ '^[0-9a-f]{64}$'
    or p_new_command_sha256 !~ '^[0-9a-f]{64}$'
  then
    raise exception using errcode = '22023', message = 'OSP_OUTBOUND_RECONCILIATION_INVALID';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      pg_catalog.json_build_array(
        p_organization_id,
        'prepare_reconciled_outbound_retry',
        p_attempt_id,
        p_job_id
      )::text,
      0
  ...
  if not exists (
    ...
    raise exception using errcode = '42501', message = 'OSP_OUTBOUND_RECONCILIATION_CONTROL_INVALID';
  end if;

  select * into prior
  from osp_private.outbound_preprovider_reconciliations evidence
  ...
  if found then
    if prior.case_id <> p_case_id
      ...
      or prior.resolution_code <> resolution
      or not exists (
        select 1
    ...
    then
      raise exception using errcode = '23505', message = 'OSP_OUTBOUND_RECONCILIATION_CONFLICT';
    end if;
  ...
  if current_case.id is null
    ...
    or not exists (
      ...
      where evidence.failed_job_id = old_job.id
        and evidence.organization_id = p_organization_id
        and evidence.case_id = p_case_id
        and evidence.attempt_id = old_attempt.id
        and evidence.job_error_code = 'INVALID_INPUT'
    )
    or exists (
      select 1
  ...
  then
    raise exception using errcode = '23514', message = 'OSP_OUTBOUND_RECONCILIATION_STALE';
  end if;
  ...
  insert into osp_private.approval_events (
    ...
    'preprovider-reconciliation', p_new_command_sha256,
  ...
  insert into osp_private.outbound_preprovider_reconciliations (
    reconciled_attempt_id, organization_id, case_id, failed_job_id,
    send_claim_token, resolution_code, gmail_absence_checked_at,
    gmail_evidence_sha256, new_attempt_id, new_job_id,
    new_idempotency_key, new_command_sha256
  ) values (
    old_attempt.id, p_organization_id, p_case_id, old_job.id,
    old_attempt.send_claim_token, resolution, p_gmail_absence_checked_at,
    p_gmail_evidence_sha256, created_attempt_id, created_job_id,
    created_idempotency_key, p_new_command_sha256
  );
...
revoke all on table osp_private.outbound_preprovider_reconciliations
from public, anon, authenticated, service_role, osp_workflow_api, osp_worker;
revoke all on function osp_private.prepare_reconciled_outbound_retry(
  uuid, uuid, uuid, uuid, uuid, timestamptz, text, text
)
from public, anon, authenticated, service_role, osp_workflow_api, osp_worker;
grant execute on function osp_private.prepare_reconciled_outbound_retry(
  uuid, uuid, uuid, uuid, uuid, timestamptz, text, text
)
to postgres;

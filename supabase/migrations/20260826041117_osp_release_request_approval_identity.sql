alter table osp_private.release_evidence_consumptions
  add column requesting_approval_key_id text,
  add column requesting_approval_key_fingerprint text;

alter function osp_private.consume_release_evidence(text, text, text, text, text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, timestamptz, timestamptz, text, text, integer)
  rename to consume_release_evidence_legacy_without_request_approval_identity;
revoke all on function osp_private.consume_release_evidence_legacy_without_request_approval_identity(text, text, text, text, text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, timestamptz, timestamptz, text, text, integer)
...
create function osp_private.consume_release_evidence(
  ...
  p_requesting_approval_key_id text,
  p_requesting_approval_key_fingerprint text,
...
begin
  if p_environment is null or p_environment <> 'production'
    ...
    or p_requesting_approval_key_id is null
    or p_requesting_approval_key_id !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{7,199}$'
    or p_requesting_approval_key_fingerprint is null
    or p_requesting_approval_key_fingerprint !~ '^[0-9a-f]{64}$'
  ...
  begin
    insert into osp_private.release_evidence_consumptions (
      ...
      requesting_approval_id, requesting_approval_role,
      requesting_approval_key_id, requesting_approval_key_fingerprint,
      issued_at, expires_at, attestation_sha256, runner_key_id,
      ...
      p_requesting_approval_role, p_requesting_approval_key_id,
      p_requesting_approval_key_fingerprint, p_issued_at, p_expires_at,
...
revoke all on function osp_private.consume_release_evidence(text, text, text, text, text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, text, text, timestamptz, timestamptz, text, text, integer)
  from public, anon, authenticated, osp_workflow_api, osp_worker, service_role, osp_release_control_runtime;
grant execute on function osp_private.consume_release_evidence(text, text, text, text, text, text, text, text, text, text, text, text, timestamptz, timestamptz, text, text, text, text, timestamptz, timestamptz, text, text, integer)
...
alter function osp_private.set_release_mode(text, text, integer, text, text, text, text, text, text, text, text, jsonb, integer, integer, jsonb)
  rename to set_release_mode_without_request_approval_identity;
revoke all on function osp_private.set_release_mode_without_request_approval_identity(text, text, integer, text, text, text, text, text, text, text, text, jsonb, integer, integer, jsonb)
  from public, anon, authenticated, osp_workflow_api, osp_worker, service_role, osp_release_control_runtime;
...
begin
  ...
  if p_release_id ~ '^osp-mvp-[0-9]{8}-[0-9]{2}$'
    ...
    where consumption.requesting_approval_key_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{7,199}$'
      and consumption.requesting_approval_key_fingerprint ~ '^[0-9a-f]{64}$'
      and consumption.requesting_approval_id is distinct from p_approval_id;
  ...
  from osp_private.set_release_mode_without_request_approval_identity(

...
begin
  ...
  target_relationship_status := case readiness_row.readiness_state
    when 'ready' then 'ready'
    when 'blocked' then 'blocked'
    else 'in_progress'
  end;
  ...
  set
    ...
    ready_at = case
      when target_status = 'ready' then coalesce(activation.ready_at, now())
      else activation.ready_at
    end,
    updated_at = now()
  where activation.organization_id = p_organization_id
    and activation.id = p_activation_id;
  ...
  set
    activation_status = target_relationship_status,
    primary_blocker = case
      when readiness_row.readiness_state = 'blocked'
        then readiness_row.blocker_requirement_codes[1]
      when readiness_row.readiness_state = 'not_configured'
        then 'activation_not_configured'
      else null
    end,
    updated_at = now()
...
begin
  ...
  if not found then
    raise exception 'Published Provider Service activation template not found for this legal entity.'
      using errcode = 'P0002';
  end if;
  ...
  select
    ...
    nullif(btrim(coalesce(p_activation_owner_user_id, '')), ''),
    case
      when template_requirement.default_due_days is null then null
...
begin
  ...
  if normalized_evidence_type !~ '^[a-z][a-z0-9_]{1,63}$'
    ...
    raise exception 'Evidence type, source system, and source reference are required.'
      using errcode = '22023';
  end if;
  ...
  select requirement.activation_id
    into activation_id
...
begin
  ...
  if normalized_state = 'not_applicable'
    ...
    raise exception 'A reviewed not-applicable decision requires an identified reviewer.'
      using errcode = '22023';
  end if;

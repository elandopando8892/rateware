-- Bind human-reviewed PDF/DOCX placement to the exact approved original.
-- Coordinates and content-control tags are never inferred by the worker.

create function osp_private.record_reviewed_native_artifact_targets_command(
  ...
  p_actor_email text,
  p_actor_permissions text[],
  p_actor_role text,
  p_authorization_session_id text,
  p_authorization_session_issued_at timestamptz
)
returns table (
  mapping_id uuid,
...
begin
  if nullif(pg_catalog.current_setting('osp.organization_id', true), '')::uuid
     ...
     or pg_catalog.jsonb_array_length(p_targets) not between 1 and 500 then
    raise exception using errcode = '42501', message = 'ARTIFACT_TARGET_REVIEW_REJECTED';
  end if;

  -- Target placement is a human Operations correction, so the immutable
  -- decision record must be backed by the same short-lived authority check as
  -- Operations review rather than by caller-supplied subject text alone.
  perform osp_private.assert_approval_actor(
    p_organization_id, 'complete_operations_review', p_actor_subject,
    p_actor_email, p_actor_permissions, p_actor_role,
    p_authorization_session_id, p_authorization_session_issued_at
  );
  ...
  where extraction.organization_id = p_organization_id
    ...
    and version.id = p_expected_source_version_id
    and version.source_sha256 = p_expected_source_sha256
  ...
  where snapshot.organization_id = p_organization_id
    ...
    and p_expected_source_version_id = any(snapshot.document_version_ids)
    and exists (
      select 1
      ...
      where ref->>'mappingId' = target_mapping.id::text
        ...
        and ref->>'extractionId' = target_mapping.extraction_id::text
        and ref->>'reviewDecisionId' = target_mapping.review_decision_id::text
    )
  ...
  for target in select item.value from pg_catalog.jsonb_array_elements(p_targets) item(value)
  loop
    if pg_catalog.jsonb_typeof(target) is distinct from 'object'
       or target->>'canonicalFieldId' !~ '^[A-Za-z][A-Za-z0-9_.-]{0,127}$' then
      raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
    end if;
    if source_version.content_type = 'application/pdf' and target->>'kind' = 'acroform' then
      if (select pg_catalog.count(*) from pg_catalog.jsonb_object_keys(target)) <> 3
         or not (target ? 'canonicalFieldId' and target ? 'kind' and target ? 'fieldName')
         or target->>'fieldName' !~ '^[^[:cntrl:]]{1,256}$'
         or pg_catalog.btrim(target->>'fieldName') <> target->>'fieldName' then
        raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
      end if;
    elsif source_version.content_type = 'application/pdf' and target->>'kind' = 'overlay' then
      if (select pg_catalog.count(*) from pg_catalog.jsonb_object_keys(target)) <> 8
         or not (target ?& array['canonicalFieldId','kind','page','x','y','width','height','fontSize'])
         or target->>'page' !~ '^[1-9][0-9]*$'
         or target->>'x' !~ '^(0|[1-9][0-9]*)([.][0-9]+)?$'
         or target->>'y' !~ '^(0|[1-9][0-9]*)([.][0-9]+)?$'
         or target->>'width' !~ '^(0[.]0*[1-9][0-9]*|[1-9][0-9]*([.][0-9]+)?)$'
         or target->>'height' !~ '^(0[.]0*[1-9][0-9]*|[1-9][0-9]*([.][0-9]+)?)$'
         or target->>'fontSize' !~ '^(0[.]0*[1-9][0-9]*|[1-9][0-9]*([.][0-9]+)?)$' then
        raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
      end if;
      if (target->>'page')::numeric > 10000
         or (target->>'x')::numeric > 100000
         or (target->>'y')::numeric > 100000
         or (target->>'width')::numeric > 100000
         or (target->>'height')::numeric > 100000
         or (target->>'fontSize')::numeric > (target->>'height')::numeric then
        raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
      end if;
    elsif source_version.content_type = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
          and target->>'kind' = 'content_control' then
      if (select pg_catalog.count(*) from pg_catalog.jsonb_object_keys(target)) <> 3
         or not (target ? 'canonicalFieldId' and target ? 'kind' and target ? 'targetTag')
         or target->>'targetTag' !~ '^[A-Za-z][A-Za-z0-9_.-]{0,127}$' then
        raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
      end if;
    else
      raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
  ...
  if (select pg_catalog.count(distinct target->>'canonicalFieldId')
      from pg_catalog.jsonb_array_elements(p_targets) item(target))
       <> pg_catalog.jsonb_array_length(p_targets)
     or (select pg_catalog.count(distinct case target->>'kind'
           when 'acroform' then target->>'fieldName'
           when 'content_control' then target->>'targetTag'
           else pg_catalog.concat_ws(':', target->>'page', target->>'x', target->>'y',
             target->>'width', target->>'height', target->>'fontSize') end)
         from pg_catalog.jsonb_array_elements(p_targets) item(target))
       <> pg_catalog.jsonb_array_length(p_targets) then
    raise exception using errcode = '23514', message = 'ARTIFACT_TARGET_INVALID';
  end if;

  select pg_catalog.jsonb_agg(target order by target->>'canonicalFieldId')
  ...
  new_payload := target_mapping.mapping_json || pg_catalog.jsonb_build_object(
    ...
    'artifactTargetSource', pg_catalog.jsonb_build_object(
      'sourceVersionId', source_version.id::text,
      'sourceSha256', source_version.source_sha256
    ),
  ...
  insert into osp_private.case_events (
    ...
    'native_artifact_targets_confirmed', extensions.gen_random_uuid(),
...
revoke all on function osp_private.record_reviewed_native_artifact_targets_command(
  uuid, uuid, uuid, integer, text, uuid, text, jsonb, text, text, text,
  text[], text, text, timestamptz
) from public, anon, authenticated, service_role;

grant execute on function osp_private.record_reviewed_native_artifact_targets_command(
  uuid, uuid, uuid, integer, text, uuid, text, jsonb, text, text, text,
  text[], text, text, timestamptz
) to osp_workflow_api;

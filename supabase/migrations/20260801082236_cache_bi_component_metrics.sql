alter table public.rateware_bi_rate_facts
  add column if not exists mx_linehaul_amount numeric,
  add column if not exists us_linehaul_amount numeric,
  add column if not exists fsc_amount numeric,
  add column if not exists border_crossing_fee_amount numeric;

update public.rateware_bi_rate_facts facts
set
  mx_linehaul_amount = public.rateware_clean_rate_number(rates.mx_linehaul),
  us_linehaul_amount = public.rateware_clean_rate_number(rates.us_linehaul),
  fsc_amount = public.rateware_clean_rate_number(rates.fsc),
  border_crossing_fee_amount = public.rateware_clean_rate_number(rates.border_crossing_fee)
...
declare
  ...
  source_mx_linehaul text;
  source_us_linehaul text;
  source_fsc text;
  source_border_fee text;
begin
  select vendor_domain, mx_linehaul, us_linehaul, fsc, border_crossing_fee
    into source_vendor_domain, source_mx_linehaul, source_us_linehaul, source_fsc, source_border_fee
  ...
  new.mx_linehaul_amount := public.rateware_clean_rate_number(source_mx_linehaul);
  new.us_linehaul_amount := public.rateware_clean_rate_number(source_us_linehaul);
  new.fsc_amount := public.rateware_clean_rate_number(source_fsc);
  new.border_crossing_fee_amount := public.rateware_clean_rate_number(source_border_fee);
  return new;
end;
$$;
...
begin
  ...
  return (
    ...
    projected as materialized (
      select dimensioned.*, coalesce(nullif(array_to_string(dimensioned.row_values, ' | '), ''), 'Total') row_key
      from dimensioned
    ),
    column_counts as (
    ...
    select jsonb_build_object(
      'rows', coalesce((select jsonb_agg(jsonb_build_object(
        'row_key', row_totals.row_key, 'row_values', to_jsonb(row_totals.row_values),
        ...
        'total', round(row_totals.total_value, 2), 'transactions', row_totals.transactions
      ...
      'row_dimensions', to_jsonb(row_dimensions), 'column_dimensions', to_jsonb(column_dimensions),

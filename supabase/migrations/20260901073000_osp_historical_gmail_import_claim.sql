create table osp_private.gmail_historical_ingest_claims (
  ...
  unique (organization_id, mailbox_email, external_message_id),
  constraint osp_historical_gmail_mailbox_exact check (
    mailbox_email = pg_catalog.lower(mailbox_email)
    and mailbox_email ~ '^[a-z0-9.!#$%&''*+/=?^_`{|}~-]+@[a-z0-9.-]+\.[a-z]{2,}$'
  ),
  constraint osp_historical_gmail_message_exact check (
    external_message_id ~ '^[A-Za-z0-9_-]{1,128}$'
    and external_thread_id ~ '^[A-Za-z0-9_-]{1,128}$'
  ),
  constraint osp_historical_gmail_subject_hash check (subject_sha256 ~ '^[0-9a-f]{64}$'),
  constraint osp_historical_gmail_sender_domain check (sender_domain ~ '^[a-z0-9.-]{1,253}$'),
  constraint osp_historical_gmail_actor check (pg_catalog.btrim(requested_by_subject) <> '')
);

alter table osp_private.gmail_historical_ingest_claims enable row level security;
alter table osp_private.gmail_historical_ingest_claims force row level security;
revoke all on osp_private.gmail_historical_ingest_claims
  from public, anon, authenticated, service_role, osp_workflow_api, osp_worker;

create policy osp_historical_gmail_tenant_select
  on osp_private.gmail_historical_ingest_claims
  for select to osp_workflow_api
  using (organization_id = nullif(current_setting('osp.organization_id', true), '')::uuid);

create policy osp_historical_gmail_tenant_insert
  on osp_private.gmail_historical_ingest_claims
  for insert to osp_workflow_api
  with check (organization_id = nullif(current_setting('osp.organization_id', true), '')::uuid);

create function osp_private.record_historical_gmail_import(

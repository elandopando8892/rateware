-- An approved release package must carry an expiry, and nothing was setting one.
--
-- provider_release_packages_approved_check requires approved_at, expires_at, and
-- expires_at > approved_at. The approval RPC set only approved_at, and the wired
-- create-package command never sets expires_at either -- so the canonical approval path
-- could not approve any package it had cut. The update failed the check every time.
--
-- Found by approving a real package end to end rather than by reading the function.
-- The TTL comes from the value recorded when the package was requested, bounded the same
-- way the command bounds it, defaulting to 24 hours.

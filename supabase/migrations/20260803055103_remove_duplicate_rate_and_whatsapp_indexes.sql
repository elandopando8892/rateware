...
begin
  ...
  if coalesce(rate_indexes_equivalent, false) is not true then
    raise exception 'Rate staging vendor indexes are missing or no longer equivalent';
  end if;
  ...
  if coalesce(whatsapp_indexes_equivalent, false) is not true then
    raise exception 'WhatsApp connection indexes are missing or no longer equivalent';
  end if;

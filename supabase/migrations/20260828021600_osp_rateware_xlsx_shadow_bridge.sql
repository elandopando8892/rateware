create table osp_private.rateware_document_bridges (
  organization_id uuid not null,
  case_id uuid not null,
  document_version_id uuid not null,
  source_background_job_id uuid not null,
  source_sha256 text not null check (source_sha256 ~ '^[0-9a-f]{64}$'),
  parser_version text not null check (parser_version = 'osp-rateware-xlsx-adjacent-label-v1'),
  raw_upload_id uuid not null unique references public.raw_uploads(id) on delete restrict,
  interpretation_job_id uuid not null unique references public.interpretation_jobs(id) on delete restrict,
  rate_staging_id uuid not null unique references public.rate_staging(id) on delete restrict,
  created_at timestamptz not null default statement_timestamp(),
  primary key (organization_id, document_version_id)
);

alter table osp_private.rateware_document_bridges enable row level security;
alter table osp_private.rateware_document_bridges force row level security;
revoke all on osp_private.rateware_document_bridges
  from public, anon, authenticated, service_role, osp_workflow_api, osp_worker;

create function osp_private.stage_rateware_xlsx_quote(
...
begin
  if p_organization_id is null or p_case_id is null or
     p_source_background_job_id is null or p_document_version_id is null or
     p_source_sha256 is null or p_source_sha256 !~ '^[0-9a-f]{64}$' or
    ...
    raise exception using errcode = '22023', message = 'INVALID_RATEWARE_XLSX_CANARY';
  end if;

  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_organization_id::text || ':' || p_document_version_id::text, 0)
  );
  ...
  if not exists (
    ...
    raise exception using errcode = '55000', message = 'SHADOW_CONTROL_REQUIRED';
  end if;
  ...
  if found then
    if v_existing.case_id <> p_case_id or
       v_existing.source_background_job_id <> p_source_background_job_id or
  ...
  if not exists (
    ...
    where job.organization_id = p_organization_id
      ...
      and pg_catalog.jsonb_typeof(job.opaque_payload) = 'string'
      and (job.opaque_payload #>> '{}')::jsonb = pg_catalog.jsonb_build_object(
        'documentVersionId', p_document_version_id
      )
  ) then
    raise exception using errcode = '22023', message = 'SOURCE_JOB_NOT_ELIGIBLE';
  end if;
  ...
  if v_source.case_id <> p_case_id or
     ...
     v_source.content_type <> 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' or
     v_source.source_sha256 <> p_source_sha256 or
     v_source.source_safety <> 'safe' then
  ...
  if exists (
    select 1
    from pg_catalog.unnest(array['vendor','rfx','origin','destination','equipment','operation','service']) candidate(key)
    ...
    select 1
    from pg_catalog.unnest(array['linehaul','borderFee','fsc','allInRate','weeklyCapacity']) candidate(key)
  ...
  insert into public.raw_uploads (
    ...
    'rate_staging',
    statement_timestamp(),
    'Synthetic OSP shadow bridge; preserved source; human review required.',
    pg_catalog.jsonb_build_object(
      'status', 'needs_review',
      'source', 'osp_shadow_bridge',
      'source_document_version_id', p_document_version_id,
      'source_sha256', p_source_sha256,
      'parser_version', p_quote->>'parserVersion'
    ),
    'needs_review',
    array['currency_missing', 'normalization_pending', 'osp_shadow_bridge'],
    1,
    1,
    1,
    statement_timestamp(),
  ...
  insert into public.interpretation_jobs (
    ...
    1,
    statement_timestamp(),
    'Deterministic OSP-to-Rateware shadow classification; no AI provider used.'
  );
...
grant execute on function osp_private.stage_rateware_xlsx_quote(
  uuid, uuid, uuid, uuid, text, jsonb
) to osp_worker;

-- Single-admin approval: let the requester approve their own release package, when the
-- package says so.
--
-- WHY
--
-- The approval path assumed two people: the requester was permanently barred from
-- approving, and the threshold defaulted to one approver who therefore had to be someone
-- else. XBF is operated by one administrator. Under that assumption no package can ever
-- be approved, so the release would happen outside the platform -- the same failure the
-- waiver work exists to prevent.
--
-- HOW, AND WHY NOT JUST DELETE THE GUARD
--
-- Deleting the check would drop separation of duties for every package forever,
-- including future ones where a second approver does exist, and the approval row would
-- then be indistinguishable from an independent one. Instead self-approval is a property
-- the package carries, chosen when the package is cut, and the approval records that it
-- was a self-approval.
--
-- Two-person approval remains the default: a package that says nothing still refuses the
-- requester.
...
-- A self-approving package cannot need more than one approval: the only eligible
-- approver may decide once, so any higher threshold is unreachable by construction and
-- would leave the package stuck looking like it was merely waiting.
alter table public.provider_onboarding_release_packages
...
-- Separation of duties was enforced twice: in the RPC and by this table constraint. That
-- redundancy is what caught an incomplete first attempt at this change, so it is kept
-- rather than dropped -- but the invariant is restated instead of removed. Requester and
-- approver may now be the same actor ONLY on a row that admits it is a self-approval, so
-- a self-approval can never be recorded as though it were an independent one.
alter table public.provider_onboarding_release_package_approvals

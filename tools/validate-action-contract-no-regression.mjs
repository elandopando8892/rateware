...
const baselineContractUrl =
  pathToFileURL(resolve(baselineRoot, "tools/effective-action-contract.mjs"))
    .href;
const { ACTION_CONTRACT: BASELINE_CONTRACT } = await import(
  `${baselineContractUrl}?build14=${Date.now()}`
);
...
const currentById = new Map(
  current.discovered.map((surface) => [surface.canonicalId, surface]),
);

console.log(
  `main: discovered=${baseline.discovered.length} errors=${baseline.normalized.length}`,
);
console.log(
  `current: discovered=${current.discovered.length} errors=${current.normalized.length}`,
);
console.log(`new authorization errors=${newErrors.length}`);

if (newErrors.length) {
  console.error(
    "Current branch introduced authorization-contract errors not present on main:",
  );
...
if (current.discovered.length !== CURRENT_CONTRACT.expectedCounts.governable) {
  throw new Error(
    `Effective contract count mismatch: discovered ${current.discovered.length}, expected ${CURRENT_CONTRACT.expectedCounts.governable}.`,
  );
}

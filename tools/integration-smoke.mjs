...
Required for authenticated checks:
  RATEWARE_E2E_AUTH_TOKEN=<Supabase access token> node tools/integration-smoke.mjs
...
Checks:
  ...
  - Supabase access token is present, current, and accepted by Rateware API.
...
const authToken = (argValue("--auth-token", process.env.RATEWARE_E2E_AUTH_TOKEN || process.env.RATEWARE_API_BEARER || "") || "").trim();
...
if (!authToken) {
  record("Supabase Auth", "fail", {
    error: "Missing RATEWARE_E2E_AUTH_TOKEN. Sign in to Rateware and provide a current Supabase access token to run authenticated production checks."
  });
  skip("Supabase API", "Requires Supabase access token.");
  skip("Gmail connection", "Requires Supabase access token.");
  skip("Google Chat OAuth connection", "Requires Supabase access token.");
  skip("Bid Room workflow", "Requires Supabase access token.");
  console.log("\nSmoke result");
...
if (!expiresAt || expiresAt <= Date.now()) {
  record("Supabase Auth", "fail", {
    error: "Supabase access token is missing exp or has expired."
  });
  console.log("\nSmoke result");
  console.log(JSON.stringify(report, null, 2));
  process.exit(1);

...
async function selectCase(id){if(!id||!detailNode)return;state.selectedId=id;renderRows();announceCase(id,safeCaseLabel(state.rows.find(r=>r.id===id)||{}));detailNode.innerHTML='<div class="onboarding-empty">Loading case workspace…</div>';try{const response=await callOsp('get_provider_onboarding_case',{case_id:id});if(state.selectedId!==id)return;renderDetail(response?.data||{});}catch(error){if(state.selectedId!==id)return;detailNode.innerHTML=`<div class="ui-state ui-state-error"><strong>Case could not load</strong><p>${escapeHtml(error?.message||'Request failed.')}</p></div>`;}}
async function loadCases({preserve=false}={}){
 ...
 try{const response=await callOsp('list_provider_onboarding_workspace',{queue:state.queue,search:state.search||undefined,limit:state.limit,offset:state.offset});if(requestId!==state.requestId)return;const data=response?.data||{};state.rows=Array.isArray(data.rows)?data.rows:[];state.total=Number(data.total||0);state.metrics=data.metrics||{};renderMetrics();renderPipeline();renderRows();renderPagination();const requested=caseFromUrl();if(!preserve&&requested&&state.rows.some(r=>r.id===requested))selectCase(requested);else if(!preserve&&state.rows[0])selectCase(state.rows[0].id);else if(state.selectedId&&!state.rows.some(r=>r.id===state.selectedId)){state.selectedId=null;detailNode.innerHTML='<div class="onboarding-empty"><strong>Select a case</strong><p>Inspect its controlled workflow.</p></div>';}}

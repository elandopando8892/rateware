...
import { fetchApprovedRateware } from "./rateware-service.js";
...
function renderRows(rows) {
  ...
  if (!rows.length) {
    body.innerHTML = '<tr><td colspan="13"><div class="empty-state"><strong>No approved rates yet</strong><span>Approve staging rows to build the Rateware.</span><a href="./staging-review.html">Open staging review</a></div></td></tr>';
    return;
  }
...
async function loadRateware() {
  body.innerHTML = '<tr><td colspan="13">Loading approved rates...</td></tr>';
  ...
  try {
    ...
    body.innerHTML = `<tr><td colspan="13">Could not load Rateware. ${escapeHtml(error.message)}</td></tr>`;
  } finally {
